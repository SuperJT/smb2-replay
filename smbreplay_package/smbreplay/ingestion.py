...
def normalize_sesid_vectorized(sesid_series: pd.Series) -> pd.Series:
    ...
    Returns:
        Series with normalized sesid lists
    """
    ...
    def normalize_single_sesid(sesid_str):
        ...
        try:
            ...
            return list(dict.fromkeys(sesids))
...
def normalize_cmd_vectorized(cmd_series: pd.Series) -> pd.Series:
    ...
    Returns:
        Series with normalized cmd lists
    """
...
def extract_unique_sessions_optimized(df: pd.DataFrame) -> List[str]:
    ...
    try:
        ...
        for sesid_str in sesid_series:
            ...
            if isinstance(sesid_str, list):
                all_sesids.extend([s.strip() for s in sesid_str if s and s != "0x0000000000000000"])
            else:
                all_sesids.extend([s.strip() for s in str(sesid_str).split(',') if s and s != "0x0000000000000000"])
...
def extract_sessions_from_dataframe_optimized(df: pd.DataFrame, unique_sesids: List[str], 
...
def save_session_metadata(case_number: str, trace_name: str, sessions: Dict[str, pd.DataFrame], 
    ...
    try:
        ...
        logger.info(f"Saved session metadata to {metadata_path} (Total memory: {total_memory:.2f}MB)")
...
def normalize_sesid(sesid_str) -> List[str]:
    ...
    return normalize_sesid_vectorized(pd.Series([sesid_str])).iloc[0]


def normalize_cmd(cmd_str) -> List[str]:
    """Legacy function for backward compatibility."""
    return normalize_cmd_vectorized(pd.Series([cmd_str])).iloc[0]
...
def run_ingestion(capture_path: Optional[str] = None, reassembly_enabled: bool = False, 
    ...
    """
    logger.info(f"Starting optimized ingestion with capture_path: {capture_path}, "
                f"reassembly_enabled: {reassembly_enabled}, force_reingest: {force_reingest}, "
                f"verbose: {verbose}")
    
    try:
        if not capture_path:
            logger.critical("No capture_path provided")
            if status_callback:
                status_callback("Error - No capture_path provided")
            return None
        ...
        packet_limit = 50000 if packet_count is not None and packet_count > 50000 else None
        
        if packet_limit:
            logger.info(f"Performance optimization: Limiting to {packet_limit} packets (total: {packet_count})")
            status_callback(f"Performance optimization: Processing first {packet_limit} packets")
        ...
        sessions = extract_sessions_from_dataframe_optimized(df, unique_sesids, status_callback)
        
        extraction_memory = psutil.Process().memory_info().rss / 1024**2
        logger.info(f"Extracted {len(sessions)} sessions (Memory: {extraction_memory:.2f} MB)")
        ...
        try:
            ...
            save_memory = psutil.Process().memory_info().rss / 1024**2
            available_memory = psutil.virtual_memory().available / 1024**2
            
            if available_memory < 512:
                logger.warning(f"Low available memory: {available_memory:.2f}MB")
                status_callback(f"Warning: Low available memory: {available_memory:.2f}MB")
        ...
        logger.info(f"Optimized ingestion completed in {elapsed_time:.2f}s "

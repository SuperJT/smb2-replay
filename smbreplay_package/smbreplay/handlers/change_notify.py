...
def handle_change_notify(replayer, op: Dict[str, Any], **kwargs):
    ...
    if not file_open:
        replayer.logger.warning(f"Change Notify: No mapping found for fid {original_fid}")
        return
    
    try:
        # Extract change notify parameters
        flags = int(op.get('smb2.changenotify.flags', 0))
        output_buffer_length = int(op.get('smb2.changenotify.output_buffer_length', 1024))
        ...
        try:
            ...
            replayer.logger.error(f"Change Notify setup failed for fid {original_fid}: {e}")
            raise
        ...
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Change Notify: Expected status {expected_status}, got success")
        ...
        expected_status = op.get('smb2.nt_status', '0x00000000')
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Change Notify: Expected status {expected_status}, got error: {e}")
    except (ValueError, TypeError) as e:
        replayer.logger.error(f"Change Notify: Invalid parameters for fid {original_fid}: {e}")
    except Exception as e:
        replayer.logger.error(f"Change Notify: Unexpected error for fid {original_fid}: {e}")
...
def _parse_change_notify_flags(flags: int) -> Dict[str, bool]:
    ...
    return {
        ...
        'watch_action_renamed_old_name': bool(flags & SMB2_WATCH_ACTION_RENAMED_OLD_NAME),
        'watch_action_renamed_new_name': bool(flags & SMB2_WATCH_ACTION_RENAMED_NEW_NAME),
    }

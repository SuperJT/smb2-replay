...
def handle_ioctl(replayer, op: Dict[str, Any], **kwargs):
    ...
    try:
        ...
        if is_fsctl:
            result = _handle_fsctl(replayer, function_code, input_data, max_output_response, op, **kwargs)
        else:
            result = _handle_device_ioctl(replayer, function_code, input_data, max_output_response, op, **kwargs)
        
        if result:
            replayer.logger.info(f"IOCTL: Successfully completed function 0x{function_code:08x}")
        else:
            replayer.logger.warning(f"IOCTL: No result for function 0x{function_code:08x}")
        ...
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"IOCTL: Expected status {expected_status}, got success")
        ...
        expected_status = op.get('smb2.nt_status', '0x00000000')
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"IOCTL: Expected status {expected_status}, got error: {e}")
...
def _handle_fsctl(replayer, function_code: int, input_data: bytes, max_output_response: int, op: Dict[str, Any], **kwargs):
    ...
    try:
        ...
        if function_code == FSCTL_CONSTANTS.get("FSCTL_QUERY_ALLOCATED_RANGES", 0x940cf):
            return _handle_query_allocated_ranges(tree, input_data, max_output_response)
        elif function_code == FSCTL_CONSTANTS.get("FSCTL_GET_NTFS_VOLUME_DATA", 0x90064):
            ...
            replayer.logger.debug(f"IOCTL: Using generic FSCTL for function 0x{function_code:08x}")
            return tree.fsctl(function_code, input_data, max_output_response)
            
    except Exception as e:
        replayer.logger.error(f"FSCTL operation failed for function 0x{function_code:08x}: {e}")
        return None


def _handle_device_ioctl(replayer, function_code: int, input_data: bytes, max_output_response: int, op: Dict[str, Any], **kwargs):
    ...
    try:
        ...
        if not tree:
            replayer.logger.warning("IOCTL: No tree available for device IOCTL operation")
            return None
        
        # Use generic IOCTL for device operations
        replayer.logger.debug(f"IOCTL: Using generic device IOCTL for function 0x{function_code:08x}")
        return tree.ioctl(function_code, input_data, max_output_response)
        
    except Exception as e:
        replayer.logger.error(f"Device IOCTL operation failed for function 0x{function_code:08x}: {e}")
        return None

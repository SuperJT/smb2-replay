...
from ..constants import SMB2_INFO_LEVELS, SMB2_FILE_INFO_CLASSES
...
def handle_query_info(replayer, op: Dict[str, Any], **kwargs):
    ...
    try:
        ...
        info_type = int(op.get('smb2.queryinfo.info_type', SMB2_INFO_LEVELS["SMB2_0_INFO_FILE"]))
        file_info_class = int(op.get('smb2.queryinfo.file_info_class', SMB2_FILE_INFO_CLASSES["SMB2_FILE_BASIC_INFO"]))
        ...
        if info_type == SMB2_INFO_LEVELS["SMB2_0_INFO_FILE"]:
            result = _query_file_info(file_open, file_info_class, output_buffer_length, additional_information)
        elif info_type == SMB2_INFO_LEVELS["SMB2_0_INFO_FILESYSTEM"]:
            result = _query_filesystem_info(file_open, file_info_class, output_buffer_length)
        elif info_type == SMB2_INFO_LEVELS["SMB2_0_INFO_SECURITY"]:
            result = _query_security_info(file_open, file_info_class, output_buffer_length, additional_information)
        ...
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Query Info: Expected status {expected_status}, got success")
        ...
        expected_status = op.get('smb2.nt_status', '0x00000000')
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Query Info: Expected status {expected_status}, got error: {e}")
    except (ValueError, TypeError) as e:
        replayer.logger.error(f"Query Info: Invalid parameters for fid {original_fid}: {e}")
    except Exception as e:
        replayer.logger.error(f"Query Info: Unexpected error for fid {original_fid}: {e}")


def _query_file_info(file_open, file_info_class: int, output_buffer_length: int, additional_information: int = 0):
...
def _query_filesystem_info(file_open, file_info_class: int, output_buffer_length: int):
    ...
    try:
        ...
        elif file_info_class == SMB2_FILE_INFO_CLASSES["SMB2_FILESYSTEM_ATTRIBUTE_INFO"]:
        ...
        elif file_info_class == SMB2_FILE_INFO_CLASSES["SMB2_FILESYSTEM_FULL_SIZE_INFO"]:
            return file_open.query_fs_full_size_info()
        elif file_info_class == SMB2_FILE_INFO_CLASSES["SMB2_FILESYSTEM_OBJECT_ID_INFO"]:
...
def _query_security_info(file_open, file_info_class: int, output_buffer_length: int, additional_information: int = 0):
    """Query security information."""
    try:
        # Security info classes are bit flags
        security_info = additional_information if additional_information else 0x00000001  # OWNER_SECURITY_INFORMATION

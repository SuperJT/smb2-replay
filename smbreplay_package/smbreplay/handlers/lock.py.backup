...
def handle_lock(replayer, op: Dict[str, Any], **kwargs):
    ...
    if not file_open:
        replayer.logger.warning(f"Lock: No mapping found for fid {original_fid}, lock command not sent.")
        return
    ...
    try:
        ...
        if not lock_elements:
            ...
            replayer.logger.debug(f"Lock field values: offset={offset}, length={length}, flags={raw_flags}")
            try:
                offset = int(offset)
            except Exception:
                offset = 0
            ...
            try:
                ...
                replayer.logger.warning(f"Could not parse lock flags '{raw_flags}', defaulting to 0: {e}")
                import pprint
                replayer.logger.error(f"Full operation for problematic lock: {pprint.pformat(op)}")
                flags = 0
        ...
        replayer.logger.info(f"Lock command(s) sent for fid={original_fid}, count={len(smb2_locks)}")

...
def handle_cancel(replayer, op: Dict[str, Any], **kwargs):
    ...
    try:
        ...
        replayer.logger.info(f"Cancel: Cancel request logged for message_id={message_id}")
        ...
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Cancel: Expected status {expected_status}, got success")
        ...
        expected_status = op.get('smb2.nt_status', '0x00000000')
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Cancel: Expected status {expected_status}, got error: {e}")

...
def handle_lease_break(replayer, op: Dict[str, Any], **kwargs):
    ...
    try:
        ...
        access_mask_hint = int(op.get('smb2.access_mask_hint', 0))
        share_mask_hint = int(op.get('smb2.share_mask_hint', 0))
        ...
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Lease Break: Expected status {expected_status}, got success")
        ...
        expected_status = op.get('smb2.nt_status', '0x00000000')
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Lease Break: Expected status {expected_status}, got error: {e}")

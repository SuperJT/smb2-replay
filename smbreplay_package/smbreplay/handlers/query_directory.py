...
def handle_query_directory(replayer, op: Dict[str, Any], **kwargs):
    ...
    if not file_open:
        replayer.logger.warning(f"Query Directory: No mapping found for fid {original_fid}")
        return
    
    try:
        # Extract query directory parameters
        file_info_class = int(op.get('smb2.querydirectory.file_info_class', FILE_INFO_CLASSES["FILE_DIRECTORY_INFORMATION"]))
        flags = int(op.get('smb2.querydirectory.flags', 0))
        file_index = int(op.get('smb2.querydirectory.file_index', 0))
        output_buffer_length = int(op.get('smb2.querydirectory.output_buffer_length', 1024))
        ...
        return_single_entry = bool(flags & SMB2_QUERY_DIRECTORY_FLAGS["SMB2_RETURN_SINGLE_ENTRY"])
        index_specified = bool(flags & SMB2_QUERY_DIRECTORY_FLAGS["SMB2_INDEX_SPECIFIED"])
        ...
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Query Directory: Expected status {expected_status}, got success")
        ...
        expected_status = op.get('smb2.nt_status', '0x00000000')
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Query Directory: Expected status {expected_status}, got error: {e}")
    except (ValueError, TypeError) as e:
        replayer.logger.error(f"Query Directory: Invalid parameters for fid {original_fid}: {e}")
    except Exception as e:
        replayer.logger.error(f"Query Directory: Unexpected error for fid {original_fid}: {e}")


def _query_directory_info(file_open, file_info_class: int, file_name: str, output_buffer_length: int,
                         restart_scan: bool = False, return_single_entry: bool = False, 
    ...
    try:
        ...
        if file_info_class == FILE_INFO_CLASSES["FILE_DIRECTORY_INFORMATION"]:
            ...
            logger.warning(f"Query Directory: Unsupported file info class {file_info_class}, using generic query")
            ...
            for entry in result_data:
                if hasattr(entry, 'file_name'):
                    entries.append({
                        ...
                        'last_access_time': getattr(entry, 'last_access_time', None),

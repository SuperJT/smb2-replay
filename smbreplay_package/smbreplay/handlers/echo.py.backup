...
def handle_echo(replayer, op: Dict[str, Any], **kwargs):
    ...
    try:
        ...
        for i in range(echo_count):
            try:
                session.echo()
                replayer.logger.debug(f"Echo: Successfully sent echo request {i+1}/{echo_count}")
            except SMBException as e:
                replayer.logger.error(f"Echo: Failed to send echo request {i+1}/{echo_count}: {e}")
                break
        ...
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Echo: Expected status {expected_status}, got success")
        ...
        expected_status = op.get('smb2.nt_status', '0x00000000')
        if expected_status and expected_status != '0x00000000':
            replayer.logger.debug(f"Echo: Expected status {expected_status}, got error: {e}")

...
class SMB2Replayer:
    ...
    def send_replay_start_ping(self, server_ip: Optional[str] = None):
        ...
        try:
            self.logger.info(
                f"🔄 Sending replay start ping to replay server: {server_ip}"
            )
    ...
    def validate_response(
        ...
        if expected_status in [None, "N/A", ""]:
            ...
            self.logger.debug(
                f"Request frame {frame_number}: Assuming expected status {expected_status}"
            )

        # Normalize status codes for comparison
        expected_hex = (
            expected_status
            if expected_status.startswith("0x")
            else (
                f"0x{int(float(expected_status)):08x}"
                if expected_status.replace(".", "", 1).isdigit()
                else expected_status
            )
        )
        actual_hex = (
            actual_status
            if actual_status.startswith("0x")
            else (
                f"0x{int(float(actual_status)):08x}"
                if actual_status.replace(".", "", 1).isdigit()
                else actual_status
            )
        )
        ...
        if status_match:
            self.logger.debug(
                f"✅ Response validation passed: {command} (Frame {frame_number}) - Status: {actual_hex}"
            )
        else:
            self.logger.warning(
                f"❌ Response validation failed: {command} (Frame {frame_number})"
            )
    ...
    def determine_create_type_and_action(
        ...
        for resp_op in all_operations:
            if (
                ...
                if create_action == "FILE_CREATED":
                    ...
                    logger.warning(
                        f"Unknown create.action '{create_action}' for msg_id {msg_id} in response frame {resp_op.get('Frame')}"
                    )
        ...
        logger.error(
            f"No matching SMB2 CREATE response found in trace for msg_id: {msg_id}"
        )
        logger.error(
            f"  Request operation frame: {json.dumps(operation, indent=2, default=str)}"
        )
        candidate_responses = [
            resp_op
            for resp_op in all_operations
        ...
        try:
            ...
            for dir_path in known_dirs:
                try:
                    ...
                    dir_open.create(
                        ...
                        | 1,  # FILE_DELETE_ON_CLOSE | FILE_DIRECTORY_FILE
                    )
                    dir_open.close()
                    files_removed += 1
                    ...
                    if "STATUS_OBJECT_NAME_NOT_FOUND" not in str(e):
                        ...
                        if "STATUS_DIRECTORY_NOT_EMPTY" in str(e):
                            logger.debug(
                                f"Directory {dir_path} is not empty, trying to remove contents"
                            )
                            ...
                            for file_path in known_files:
                                if file_path.startswith(dir_path + "\\"):
                                    try:
                                        ...
                                        file_open.close()
                                        logger.debug(
                                            f"Removed file inside directory: {file_path}"
                                        )
                                    except SMBException as fe:
                                        logger.debug(
                                            f"Could not remove {file_path}: {fe}"
                                        )
            ...
            for dir_path in known_dirs:
                try:
                    ...
                    if "STATUS_OBJECT_NAME_NOT_FOUND" not in str(e):
                        logger.debug(
                            f"Could not remove directory {dir_path} (second attempt): {e}"
                        )
    ...
    def setup_pre_trace_state(
        ...
        logger.info(
            f"Pre-trace: will create {len(directories)} directories and {len(file_paths)} files if missing."
        )
        ...
        try:
            logger.info(
                "Closing SMB session and connection after pre-trace setup to force new establishment for replay."
            )
            try:
                tree.disconnect()
            except Exception as e:
    ...
    def _validate_file_system_structure(
        self, tree: TreeConnect, paths: set, created_dirs: set
    ):
        """
        ...
        if missing_dirs:
            ...
            logger.warning(
                f"Only {accessible_paths}/{len(paths)} paths will be accessible during replay"
            )
        else:
    ...
    def handle_response(self, op: Dict[str, Any], cmd: int):
        ...
        if cmd == 3:  # Tree Connect response
            ...
            if self.state["last_new_tid"] is not None:
                ...
                logger.debug(
                    f"Mapped tid {original_tid} to {self.state['last_new_tid']}"
                )
            ...
            if self.state["last_new_fid"] is not None:
                ...
                logger.debug(
                    f"Mapped fid {original_fid} to {self.state['last_new_fid']}"
                )
    ...
    def handle_change_notify(self, op: Dict[str, Any]):
        ...
        if file_open:
            try:
                ...
                logger.debug(
                    f"Change Notify: fid={original_fid}, Open object={file_open}"
                )
        ...
        try:
            ...
            for op in selected_operations:
                try:
                    ...
                    cmd_counter["error"] += 1
            logger.info(
                f"Main replay: processing {len(selected_operations)} operations. Command summary: {dict(cmd_counter)}"
            )

            logger.debug(
                f"Main replay: processing {len(selected_operations)} operations"
            )
            ...
            for i, op in enumerate(selected_operations, 1):
                logger.debug(
                    f"Replay loop: operation {i}/{len(selected_operations)}: cmd={op.get('smb2.cmd')} resp={op.get('smb2.flags.response')} frame={op.get('Frame')} name={op.get('Command')}"
                )
                try:
                    cmd_raw = op.get("smb2.cmd", "-1")
                    cmd = int(cmd_raw) if str(cmd_raw).isdigit() else -1
                ...
                try:
                    ...
                    if status_callback:
                        status_callback(
                            f"Processing operation {i}/{len(selected_operations)}: {op.get('Command', 'Unknown')}"
                        )
            ...
            if status_callback:
                status_callback(
                    f"Replay completed: {successful_ops} successful, {failed_ops} failed"
                )
                if validation_results["enabled"]:
                    status_callback(
                        f"Response validation: {validation_results['matching_responses']}/{validation_results['total_operations']} responses match ({validation_results['match_rate']:.1f}%)"
                    )
    ...
    def command_handlers(self):
        ...
        return {
            ...
            5: lambda tree, op, all_operations=None: handle_create(
                self, tree, op, all_operations
            ),
    ...
    def validate_operations(self, operations: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_operations": sum(
                1
                for op in operations
                if int(op.get("smb2.cmd", -1)) in supported_commands
            ),
        }

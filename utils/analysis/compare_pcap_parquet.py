...
def run_tshark_on_pcap(pcap_path: str, session_id: str | None = None) -> pd.DataFrame:
    ...
    try:
        ...
        if session_id and not df.empty and "smb2.sesid" in df.columns:
            ...
            print(
                f"Filtered from {original_count} to {len(df)} frames for session {session_id}"
            )
...
def compare_tree_connect_frames(
    pcap_df: pd.DataFrame, parquet_df: pd.DataFrame
) -> None:
    ...
    if len(pcap_tree_connects) > 0:
        ...
        for idx, row in pcap_tree_connects.iterrows():
            ...
            print(
                f"  Frame {frame_num}: cmd={cmd}, response={is_response}, tid={tid}, tree={tree_path}"
            )
...
def compare_command_distribution(
    pcap_df: pd.DataFrame, parquet_df: pd.DataFrame
) -> None:
...
def compare_frame_ranges(pcap_df: pd.DataFrame, parquet_df: pd.DataFrame) -> None:
    ...
    if "frame.number" in parquet_df.columns and len(parquet_df) > 0:
        ...
        print(
            f"Parquet frame range: {parquet_min} - {parquet_max} ({len(parquet_df)} frames)"
        )
    else:
...
def main():
    if len(sys.argv) not in [2, 3]:
        ...
        print(
            "  python compare_pcap_parquet.py /path/to/capture.pcap /path/to/tshark_output_full.parquet"
        )
        sys.exit(1)
    ...
    if len(sys.argv) == 3:
        ...
        if second_arg.endswith(".parquet"):
            ...
            if not parquet_path:
                print(
                    f"Error: Could not find full parquet file for session {session_id}"
                )
                print("Please provide the full parquet file path directly:")
                print(
                    "  python compare_pcap_parquet.py <pcap_file> <full_parquet_file>"
                )
                sys.exit(1)
    else:
    ...
    print("Comparing PCAP and full parquet file")
    if session_id:

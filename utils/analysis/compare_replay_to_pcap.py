...
def extract_smb_operations_from_pcap(
    pcap_path: str, session_id: str | None = None
) -> list[dict[str, Any]]:
...
def compare_operations(
    original_ops: list[dict[str, Any]], replayed_ops: list[dict[str, Any]]
) -> dict[str, Any]:
    ...
    original_in_session = {
        msg_id: op
        for msg_id, op in original_by_msg_id.items()
        if msg_id in session_msg_ids
    }
    ...
    if matching_ids:
        ...
        print(
            f"   Debug - Original message ID examples: {list(original_by_msg_id.keys())[:5]}"
        )
        ...
        if msg_id in original_in_session and msg_id in replayed_by_msg_id:
            ...
            for field in fields_to_compare:
                ...
                if orig_val != replay_val:
                    differences[field] = {"original": orig_val, "replayed": replay_val}
...
def analyze_response_patterns(original_ops: list[dict[str, Any]]) -> dict[str, Any]:
                ...
                create_operations.append(
                    {
                        ...
                        "create_disposition": request.get(
                            "smb2.create_disposition", "N/A"
                        ),
    ...
    return {
        ...
        "total_pairs": len(
            [
                p
                for p in request_response_pairs.values()
                if p["request"] and p["response"]
            ]
        ),
    }
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print(
            "Example: python compare_replay_to_pcap.py smb2_session_0x9dbc000000000006.parquet"
        )
        sys.exit(1)
    ...
    print("🔍 Testing session ID filter...")
    test_cmd = [
        "tshark",
        "-r",
        pcap_path,
        "-Y",
        f"smb2 && smb2.sesid == {session_id}",
        "-T",
        "fields",
        "-e",
        "frame.number",
    ...
    if not test_result.stdout.strip():
        print("⚠️  No results with session ID filter, trying without session filter...")
        session_id = None
    ...
    print(
        f"✅ Replay completed: {results['successful_operations']}/{results['total_operations']} operations"
    )
    ...
    print("=" * 20)
    print(
        f"📁 Session file contains {comparison['total_replayed']} operations from {comparison['total_original']} total PCAP operations"
    )
    print("🎯 This represents the filtered SMB session for replay analysis")

    if comparison["match_rate"] > 90:
        print(
            "✅ Excellent session match rate - replay is very faithful to original session"
        )
    elif comparison["match_rate"] > 70:
        print("⚠️  Good session match rate - some differences but mostly accurate")
    else:
        print(
            "❌ Low session match rate - significant differences from original session"
        )

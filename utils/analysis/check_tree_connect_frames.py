    ...
    try:
        ...
        for frame_num in frame_numbers:
            ...
            if len(matching_rows) > 0:
                ...
                for idx, row in matching_rows.iterrows():
                    ...
                    print(
                        f"  Session: {sesid}, Cmd: {cmd}, TID: {tid}, Tree: {tree}, Response: {response}"
                    )
            else:
        ...
        if len(tree_connects) > 0:
            for idx, row in tree_connects.iterrows():
                ...
                print(
                    f"  Frame {frame_num}: Session: {sesid}, Cmd: {cmd}, TID: {tid}, Tree: {tree}, Response: {response}"
                )
        else:
...
def main():
    ...
    session_parquet = os.path.join(
        ...
        trace_name,
        "sessions",
        "smb2_session_0x9dbc000000000006.parquet",
    )
    ...
    other_session_parquet = os.path.join(
        cases_dir,
        case_id,
        ".tracer",
        trace_name,
        "sessions",
        "smb2_session_0x00012c01c400000d.parquet",
    )

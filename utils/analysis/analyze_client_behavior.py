...
def analyze_client_behavior(df: pd.DataFrame) -> dict[str, Any]:
    ...
    for msg_id in df["smb2.msg_id"].unique():
        ...
        request = (
            frames[frames["smb2.flags.response"] == "False"].iloc[0]
            if len(frames[frames["smb2.flags.response"] == "False"]) > 0
            else None
        )
        response = (
            frames[frames["smb2.flags.response"] == "True"].iloc[0]
            if len(frames[frames["smb2.flags.response"] == "True"]) > 0
            else None
        )
                ...
                create_operations.append(
                    {
                        ...
                        "impersonation_level": request.get(
                            "smb2.impersonation_level", "N/A"
                        ),
                        "response_status": response.get("smb2.nt_status", "N/A"),
                        "response_action": response.get("smb2.create.action", "N/A"),
                        "response_status_desc": response.get(
                            "smb2.nt_status_desc", "N/A"
                        ),
                    }
                )
    ...
    successful_creates = [
        op for op in create_operations if op["response_status"] == "0x00000000"
    ]
    failed_creates = [
        op for op in create_operations if op["response_status"] != "0x00000000"
    ]
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print(
            "Example: python analyze_client_behavior.py smb2_session_0x9dbc000000000006.parquet"
        )
        sys.exit(1)
    ...
    for i, op in enumerate(analysis["create_operations"][:10], 1):
        ...
        print(
            f"   Disposition: {op['create_disposition']} ({interpret_create_disposition(op['create_disposition'])})"
        )
        print(
            f"   Options: {op['create_options']} ({interpret_create_options(op['create_options'])})"
        )
        print(f"   File Attributes: {op['file_attributes']}")
        print(f"   Desired Access: {op['desired_access']}")
        print(f"   Share Access: {op['share_access']}")
        print(
            f"   Response Status: {op['response_status']} ({op['response_status_desc']})"
        )
    ...
    for filename, intent in list(analysis["client_intent"].items())[:10]:
        print(f"\n📄 {filename}")
        print(
            f"   First disposition: {intent['first_disposition']} ({interpret_create_disposition(intent['first_disposition'])})"
        )
        ...
        if all(d == "3" for d in dispositions):
            print("   Intent: CREATE_NEW (client wanted to create new file)")
        elif all(d == "2" for d in dispositions):
            print("   Intent: OPEN_EXISTING (client wanted to open existing file)")
        elif all(d == "4" for d in dispositions):
            print(
                "   Intent: OPEN_OR_CREATE (client wanted to open if exists, create if not)"
            )
        else:
            print("   Intent: MIXED (client tried different approaches)")

...
def analyze_workflow_state(df: pd.DataFrame) -> dict[str, Any]:
    ...
    file_states: defaultdict[str, dict[str, Any]] = defaultdict(
        lambda: {
            "created": False,
            "opened": False,
            "first_seen": None,
    ...
    dir_states: defaultdict[str, dict[str, Any]] = defaultdict(
        lambda: {
            "created": False,
            "accessed": False,
            "first_seen": None,
    ...
    return {"files": dict(file_states), "directories": dict(dir_states)}
...
def predict_expected_responses(df: pd.DataFrame) -> dict[str, Any]:
        ...
        if not is_response and cmd == "5":  # Create request
            ...
            if prev_frame in file_exists_at_frame:
                ...
                if file_exists or dir_exists:
                    ...
                    expected_action = (
                        "FILE_OPENED" if not dir_exists else "DIRECTORY_OPENED"
                    )
                else:
    ...
    return {"predictions": predictions, "workflow_state": workflow_state}
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print(
            "Example: python analyze_workflow_state.py smb2_session_0x9dbc000000000006.parquet"
        )
        sys.exit(1)
    ...
    print(
        f"   Directories created: {sum(1 for d in directories.values() if d['created'])}"
    )
    print(
        f"   Directories accessed: {sum(1 for d in directories.values() if d['accessed'])}"
    )

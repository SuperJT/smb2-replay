...
def capture_setup_pcap(session_id, pcap_file=None):
    ...
    try:
        ...
        setup_process = subprocess.run(setup_cmd, capture_output=False, text=True)
        ...
        print(
            f"✅ Setup command completed with return code: {setup_process.returncode}"
        )
        print(f"📁 PCAP file saved: {pcap_file}")
        print("💡 Analyze the PCAP to see what SMB operations were performed")
        print(f"💡 Use: tshark -r {pcap_file} -Y 'smb2' to filter SMB2 traffic")
        print(
            f"💡 Use: tshark -r {pcap_file} -Y 'smb2.cmd == 5' to see Create operations"
        )
...
def main():
    ...
    parser.add_argument(
        "session_id", help="Session ID to setup (e.g., 0x9dbc000000000006)"
    )

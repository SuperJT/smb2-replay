...
def smb_env():
    ...
    print(
        f"[pytest] Attempting SMB connection to {SERVER_IP}:445 as {USERNAME} on share {SHARE_NAME}"
    )
    ...
    if not hasattr(connection, "transport") or connection.transport is None:
        pytest.fail(
            f"SMB connection.transport is None after connect() to {SERVER_IP}:445"
        )
    try:
...
@pytest.mark.parametrize(
    "create_type, disposition, expect_directory",
    [
...
def test_create_object(smb_env, create_type, disposition, expect_directory):
    ...
    try:
        ...
        if expect_directory:
            handle.query_directory("*", FileInformationClass.FILE_NAMES_INFORMATION)
        else:
            handle.read(0, 1)
...
@pytest.mark.parametrize(
    "options",
    [CreateOptions.FILE_DIRECTORY_FILE, CreateOptions.FILE_NON_DIRECTORY_FILE, 0],
)
...
def test_directory_listing(smb_env):
    ...
    entries = list_handle.query_directory(

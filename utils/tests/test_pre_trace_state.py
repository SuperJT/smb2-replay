...
def test_pre_trace_state():
    ...
    try:
        ...
        print(f"Found {len(directories)} directories and {len(normalized_paths)} files to process")
        print(f"Directories to create: {sorted(directories, key=lambda x: (x.count('\\'), x))}")
        ...
        print(f"✅ Pre-trace state setup complete:")
        ...
        if len(created_dirs) < len(directories):
            print(f"⚠️  Only {len(created_dirs)}/{len(directories)} directories could be created")
            print("⚠️  Some nested directories may not exist - replay may fail for those paths")
            print("ℹ️  Consider using a different SMB server or share that supports nested directory creation")
...
def validate_file_system_structure(paths: set, created_dirs: set):
    ...
    if missing_dirs:
        ...
        for missing_dir in sorted(missing_dirs):
            print(f"    - {missing_dir}")
        print(f"⚠️  Only {accessible_paths}/{len(paths)} paths will be accessible during replay")
    else:

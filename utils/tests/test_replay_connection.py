    ...
    try:
        ...
        if result.get("success"):
            ...
            raise AssertionError(
                f"Replay system test failed: {result.get('error', 'Unknown error')}"
            )
    ...
    if config_ok:
        ...
        if connection_ok:
            ...
            if replay_ok:
                ...
                print(
                    "2. Ingest a PCAP: smbreplay ingest --case <case_id> --trace <file.pcap>"
                )
                print("3. List sessions: smbreplay list sessions --case <case_id>")
                print(
                    "4. Replay a session: smbreplay replay <session_id> --case <case_id>"
                )
            else:

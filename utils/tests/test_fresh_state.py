...
def test_fresh_state_reset():
    ...
    try:
        ...
        session_frames, field_options, file_options, selected_fields = (
            sm.load_and_summarize_session(capture_path, session_file)
        )
        ...
        supported_operations = [
            ...
            if op.get("smb2.cmd") in ["3", "5"]
            and op.get("smb2.flags.response") == "False"
        ]
        ...
        for op in test_operations:
            print(
                f"  - {op.get('Command', 'Unknown')} (cmd={op.get('smb2.cmd', 'N/A')})"
            )
            ...
            if validation["mismatched_responses"] > 0:
                ...
                for detail in validation["details"]:
                    if not detail["status_match"]:
                        print(
                            f"  Frame {detail['frame']}: {detail['command']} ({detail['filename']})"
                        )
        ...
        assert results["success"] and results2["success"], (

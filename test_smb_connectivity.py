...
def test_tree_connect(conn: SMBConnection, tree_name: str) -> Optional[str]:
...
def test_directory_listing(conn: SMBConnection, tree_name: str, path: str = '/') -> bool:
...
def test_file_operations(conn: SMBConnection, tid: str, test_filename: str = 'smb2_replay_test.txt') -> bool:
...
def main():
    ...
    if not test_directory_listing(conn, config.get_tree_name()):
